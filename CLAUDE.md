...
- **Build status:** Milestones 1–3 done (converter + local images + incremental change-detection; verified against the real export — re-sync = 68 unchanged, 0 downloads). Next (see `docs/ghost-to-vault.md` §7): sparse JSON-archive retention (M4), scripted export via session auth (M5), RSS watch (M6).
