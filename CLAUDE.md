...
- **Command:** `node src/cli.js sync --from <export.json> [--out <dir>] [--dry-run] [--drafts]`. **Always** test with `--dry-run` (writes `out/vault-preview/`); only launchd writes the real `VAULT_DIR`.
- **Build status:** Milestone 1 done (converter MVP — markdown + frontmatter + layout; images still as Ghost URLs; verified against the real export, 68 posts). Next milestones (see `docs/ghost-to-vault.md` §7): local image download, idempotency+git, sparse JSON-archive retention, scripted export (Tier 2), RSS watch (Tier 3).
