...
- **Images:** only `http(s)` srcs are downloaded into each post's folder as `<slug>__feature.<ext>` / `<slug>__<n>.<ext>`; markdown + `feature_image` rewritten to bare local filenames. The `src=` parser uses a matched-quote capture so **literal apostrophes** in URLs (e.g. `…/A-380's….png`, from Era-1 GitHub hotlinks — `encodeURIComponent` leaves `'` unescaped) don't truncate. Non-URL refs (relative `../attachments/…` that were never rewritten because the source image was missing) are **left as-is and reported as broken**, not fetched. Idempotent; same URL reused across posts fetched once then copied. See `src/vault/images.js` + `convert.js`.
...
- **Build status:** Milestones 1–4 done & verified; M5 (scripted export) built but **not verified live** (creds/2FA/rate-limit). Next: RSS watch (M6) — only worth it if fetch-export actually works for this account (i.e., 2FA off or a no-2FA export account).
